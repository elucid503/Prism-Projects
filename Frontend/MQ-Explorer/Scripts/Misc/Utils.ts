...
export async function LogToBackend(Message: string): Promise<boolean> {

    const Response = await MakeRequest(Routes.Report, { Message });

    if (!Response) return false;

    return Response.Success ?? false; // Important: using the nullish whatever operator to avoid undefined values
